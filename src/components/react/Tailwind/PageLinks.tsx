  ...
  return (
    <article key={page.slug}>
      <Border position="left" className="relative flex flex-col items-start pl-8">
        <h3 className="mt-6 text-base font-semibold text-neutral-950">{page.data.title}</h3>
        {/* <time dateTime={page.date} className="order-first text-sm text-neutral-600">
          {formatDate(page.date)}
        </time> */}
        <p className="mt-2.5 text-base text-neutral-600">{page.data.description}</p>
        <a
          href={page.slug}
          className="mt-6 flex gap-x-3 text-base font-semibold text-neutral-950 transition hover:text-neutral-700"
          aria-label={`Read more: ${page.data.title}`}
        >
          Read more
